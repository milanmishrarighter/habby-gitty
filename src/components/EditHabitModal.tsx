...
const EditHabitModal: React.FC<EditHabitModalProps> = ({ isOpen, onClose, initialHabit, onSave }) => {
  ...
  React.useEffect(() => {
    if (initialHabit) {
      ...
      setHabitType(initialHabit.type || 'tracking'); // Set habit type
      ...
      setAllowedOutOfControlMisses(initialHabit.allowedOutOfControlMisses === 0 ? "" : initialHabit.allowedOutOfControlMisses);
      setHintText(initialHabit.hintText || "");
    } else {
  ...
  const handleSave = () => {
    ...
    const updatedHabit: Habit = {
      ...
      ...(habitType === 'tracking' ? {
        ...
        allowedOutOfControlMisses: typeof allowedOutOfControlMisses === 'number' ? allowedOutOfControlMisses : 0,
      } : { // If type is 'text_field', ensure these are undefined/default
        trackingValues: undefined,
        frequencyConditions: undefined,
        fineAmount: undefined,
        yearlyGoal: undefined,
        allowedOutOfControlMisses: undefined,
      }),
    };
