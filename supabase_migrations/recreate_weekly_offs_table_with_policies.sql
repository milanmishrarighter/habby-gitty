DO $$
BEGIN
    -- Check if the table exists before attempting to drop policies
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'weekly_offs') THEN
        DROP POLICY IF EXISTS "Enable read access for all users" ON public.weekly_offs;
        DROP POLICY IF EXISTS "Enable insert for authenticated users only" ON public.weekly_offs;
        DROP POLICY IF EXISTS "Enable update for authenticated users only" ON public.weekly_offs;
        DROP POLICY IF EXISTS "Enable delete for authenticated users only" ON public.weekly_offs;
    END IF;

    -- Drop the table if it exists, and cascade to remove any dependent objects (like policies)
    -- This ensures a clean slate for table creation.
    DROP TABLE IF EXISTS public.weekly_offs CASCADE;

    -- Create the weekly_offs table
    CREATE TABLE public.weekly_offs (
        id uuid DEFAULT gen_random_uuid() NOT NULL,
        year text NOT NULL,
        week_number integer NOT NULL,
        is_off boolean DEFAULT false NOT NULL,
        created_at timestamp with time zone DEFAULT now() NOT NULL
    );

    -- Add primary key constraint
    ALTER TABLE public.weekly_offs ADD CONSTRAINT weekly_offs_pkey PRIMARY KEY (id);

    -- Add unique constraint for year and week_number
    ALTER TABLE public.weekly_offs ADD CONSTRAINT weekly_offs_year_week_number_key UNIQUE (year, week_number);

    -- Enable Row Level Security (RLS) for the table
    ALTER TABLE public.weekly_offs ENABLE ROW LEVEL SECURITY;

    -- RLS Policies for weekly_offs table

    -- Policy to allow authenticated users to read their own data
    CREATE POLICY "Enable read access for all users" ON "public"."weekly_offs"
    AS PERMISSIVE FOR SELECT
    TO public
    USING (true);

    -- Policy to allow authenticated users to insert data
    CREATE POLICY "Enable insert for authenticated users only" ON "public"."weekly_offs"
    AS PERMISSIVE FOR INSERT
    TO authenticated
    WITH CHECK (true);

    -- Policy to allow authenticated users to update their own data
    CREATE POLICY "Enable update for authenticated users only" ON "public"."weekly_offs"
    AS PERMISSIVE FOR UPDATE
    TO authenticated
    USING (true)
    WITH CHECK (true);

    -- Policy to allow authenticated users to delete their own data
    CREATE POLICY "Enable delete for authenticated users only" ON "public"."weekly_offs"
    AS PERMISSIVE FOR DELETE
    TO authenticated
    USING (true);

END $$;